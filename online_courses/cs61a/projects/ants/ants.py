...
class Ant(Insect):
    ...
    def add_to(self, place):
        ...
        if place.ant is None:
            place.ant = self
        elif self.is_container and place.ant.is_container:
            assert 0, 'Two ants in {0}'.format(place)
        elif place.ant.is_container and place.ant.can_contain(self):
            place.ant.store_ant(self)
        elif self.is_container:
            curant = place.ant
            self.store_ant(curant)
            place.ant = self
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not cur_place.is_hive:
            if not self.lower_bound <= cur_state <= self.upper_bound:
                pass
            elif cur_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if lst:
            for bee in lst:
                if self.health == 0:
                    Bee.reduce_health(bee, amount + self.damage)
                else:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0 and self.place.bees:
            curbee = random_bee(self.place.bees)
            healths = curbee.health
            curbee.reduce_health(healths)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        if self.place.bees:
            lst = list(self.place.bees)
        for tbee in lst:
            tbee.reduce_health(self.damage)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # double the damage behind the queen ant
        curplace = self.place.exit
        while curplace:
            if curplace.ant:
                curant = curplace.ant
                if not curant.doubled:
                    curant.doubled = True
                    curant.damage *= 2
                if curant.is_container and curant.ant_contained:
                    if not curant.ant_contained.doubled:
                        curant.ant_contained.doubled = True
                        curant.ant_contained.damage *= 2
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.place.bees:
            lst = list(self.place.bees)
            for curbee in lst:
                curbee.reduce_health(self.damage)
        # END Problem Optional 1
