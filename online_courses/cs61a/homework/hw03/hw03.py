...
def interleaved_sum(n, odd_term, even_term):
    ...
    def adds_to_n(t, f, g):
        if t == n:
            return f(n)
        return f(t) + adds_to_n(t + 1, g, f)

    return adds_to_n(1, odd_term, even_term)
...
def count_coins(total):
    ...
    >>> count_coins(20)
    9
    >>> count_coins(1000) # How many ways to make change for a dollar?
    242
    >>> count_coins(200)
    1463
    >>> from construct_check import check
    ...
    "*** YOUR CODE HERE ***"
    # 1 5 10 25
    def count_coins_with_max(value, max_num):
        if value < 0:
            return 0
        if value == 0:
            return 1
        if max_num == 1:
            return 1
        return count_coins_with_max(value - max_num, max_num) + count_coins_with_max(value, next_smaller_coin(max_num))
    return count_coins_with_max(total, 25)
